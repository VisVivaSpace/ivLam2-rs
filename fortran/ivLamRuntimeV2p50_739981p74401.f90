...
!                   -December 2025, RPR: updated the stopping condition for root-solve to be
!                   compatible with gfort, fixed formating for gfort to compile.
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
!---------------------------------------------------------------------------------------------------    
...
!                   Release 2.50, 09-17-2021,  updated headers with the DOI of the accepted paper [2].  Compiled with Intel(R) Visual Fortran Compiler 19.0.3.203 [Intel(R) 64] on Windows 10. This release is archived: DOI: 10.5281/zenodo.5196639    
!---------------------------------------------------------------------------------------------------    
!CODE DESCRIPTION
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  
    ...
    type interpLamberInput
        real(kind=ru):: deltaVarTol_mrev =1.0d0  !nominal value is 1.0d0, could go up to 1e-2, applies to all multi-rev cases  !12-30-2025 changed all these to 1.0 when added stopCase=1
        real(kind=ru):: deltaVarTol_zrevB=1.0d0   !nominal value is 1.0d0, recommend this one to not change, applies to zero rev cases when tau<0 and parabolic
        real(kind=ru):: deltaVarTol_zrevA=1.0d0   !nominal value is 1.0d0, could go up to 1e-2, applies to all other zero rev cases
                                          ...
                                          !it's tied to the stopping mechanism for the iterative solver, see #289999056
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  
    ...
    !############################################################ 
        ...
        integer(kind=iu),parameter:: stopCase=1 !0 or 1   !see #289999056       
        real(kind=ru),parameter:: eps1=epsilon(1.0_ru),epsSQRT=sqrt(eps1)
        real(kind=ru) tempVar,relerr
        logical:: yesStop
        ...
        do i=1,ivLamParam_maxIters  !do an update to ksol, high order step is smart rather than multiple single order steps because the expensive part is computing W(0), all the partials are recursive or simple algebra otherwise  
            ...
            if(ivLamParam_maxIters>1)  then !ivLamParam_maxIters should be a compile time decision, then no performance hit for if statement (i.e. its cooked in at compile time)
                if(infoCorr==0) then   ! 
                    ...
                    select case(stopCase) !#289999056   !rprmod 12-30-2025 update
                        case(0) !original one, liam healy found a problem with this when compiled with gfort in his setup, this one works fine with ifort, but its bad practice to use a==b when a and b are floats.  
                                        yesStop=(bert%dvars(ivLamParam_orderCorrection)*deltaVarTol+compVal==compVal) 
                        case(1)
           
                            tempVar= bert%dvars(ivLamParam_orderCorrection)*deltaVarTol     !want to stop when this gets small

                            !need to normalize by epsSQRT (or 1.0_ru or eps1) if the magnitude is big; 2 is recommended, seems a good compromise.
                            select case(2)
                                case(2)
                                    relerr= abs( (tempVar)) /max(epsSQRT,abs(compVal)  )   
                                case(1)
                                    relerr= abs( (tempVar)) /max(1.0_ru,abs(compVal)  )   
                                case(0)
                                    relerr= abs( (tempVar)) /max(eps1,abs(compVal)  )   
                            end select
                            yesStop=(relerr <eps1 ) 
                        case default
                            write(prntU,*) '**problem invalid stopCase, dev problem',stopCase;stop
                    end select
        ...
        !print*,'bert%iters=',bert%iters,bert%dvars(ivLamParam_orderCorrection)        
  ...
  !
    ...
    !############################################################   
        ...
        if( (mRev%lims(1,1).ne.zRev%xlow(1)).or.(mRev%lims(2,1).ne.zRev%xhi(1))  ) then
            write(prntU,'(a,100d26.16)') '**error, zero and mrev should have have same xlims=',mRev%lims(1,1),zRev%xlow(1),mRev%lims(1,2),zRev%xhi(1);info=-6;return   
        endif
        ...
        if(1==2) then !to print details, really for developers
            ...
            write(prntU,'(a100,100d14.5)') 'max normalized errors on the interpolations for TbyS_MultiRevBottom, k_MultiRev, k_ZeroRev=',mRev%kbotMaxval,mRev%maxval,zRev%maxval  
        endif
